...
engine = pyttsx3.init()
face_cascade = cv2.CascadeClassifier('haarcascade_frontalface_default.xml') #подключаем каскадные фильтры
face_cascade_nofront = cv2.CascadeClassifier("haarcascade_profileface.xml") #подключаем каскадные фильтры
...
class Cam:
    ...
    def __init__(self, master=None):
        ...
        self.master = master
        self.canvas = Canvas(master, width=500, height=400) #создаём канвас для изображение с камеры
    ...
    def photo(self):
        ...
        if ret:
            ...
            for (x, y, w, h) in faces: #находим лица
                cv2.rectangle(frame, (x, y), (x + w, y + h), (250, 0, 0), 2)#обводим лица
...
def procent(Cam):
    ...
    if ret:
        frame = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)# ргб изображение с камеры
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)# серое изображение с камеры
        faces = face_cascade.detectMultiScale(gray, 1.3, 5) #обнаруживаем лица
        print(faces)
        ...
        print(photo)
        for i in photo:
            image_1 = im_crop #фото с видиопотока
            image_2 = Image.open("photo/1/" + i) #фото из баз
            result = ImageChops.difference(image_1, image_2) #накладываем их друг на друга
            f = 0
            ...
            for i in test:
                for j in i:
                    if j[0] < 30 and j[1] < 30 and j[2] < 30:
                        f += 1
                    else:
                        continue
            x = 100 * f // size
